...
async function performAnalysis() {
  ...
  var loadingMessages = [
    ...
    'Reviewing skin and features...',
    'Checking hydration and sleep indicators...',
    'Putting together your suggestions...',
    'Almost there...'
  ];
  var msgIndex = 0;
  status.innerHTML = '<div class="loading-box"><div class="spinner"></div><div class="loading-text">' + loadingMessages[0] + '</div></div>';
...
function displayResults(data) {
  ...
  function makeSection(title, observation, suggestion) {
    var s = '<div class="result-section">';
    s += '<div class="result-section-title">' + title + '</div>';
    if (observation) s += '<div class="result-observation">' + observation + '</div>';
    if (suggestion) s += '<div class="result-suggestion">' + suggestion + '</div>';
    s += '</div>';
    return s;
  }

  if (typeof data === 'string') {
    html = '<div class="result-observation">' + data + '</div>';

  } else if (data.skin || data.eyes || data.hydration || data.overall) {
    // Direct tool output from Claude (wellness_analysis tool response)
    var disclaimerHtml = '';
    if (data.disclaimer) {
      disclaimerHtml = '<div class="result-disclaimer">⚠️ ' + data.disclaimer.toUpperCase() + '</div>';
    }
    // Disclaimer at top
    html += disclaimerHtml;
    ...
    sections.forEach(function (sec) {
      if (data[sec.key]) {
        html += makeSection(sec.label, data[sec.key].observation, data[sec.key].suggestion);
      }
    });
    ...
    Object.keys(data.report).forEach(function (key) {
      ...
      if (typeof value === 'object' && !Array.isArray(value)) {
        html += makeSection(label, value.observation, value.suggestion);
      } else if (Array.isArray(value)) {
        html += makeSection(label, null, value.join('<br>'));
      } else {
        html += makeSection(label, value, null);
      }
    });

  } else {
    html = '<div class="result-observation">' + JSON.stringify(data, null, 2) + '</div>';
  }
